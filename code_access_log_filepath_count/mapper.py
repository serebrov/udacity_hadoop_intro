...
#
# For this task we need the number of hits for each file path,
...
# /assets/js/lowpro.js
# /displaytitle.php?id=http://www.npk-spb.ru/images/circle.gif?
# /displaytitle.php?id=http://www.npk-spb.ru/images/circle.gif?
# http://www.the-associates.co.uk/displaytitle.php?id=537
regex_url = '(http:\/\/[^\/]+)?([^\?]*)(\?.*)?'
r_url = re.compile(regex_url)

for line in sys.stdin:
    matches = r.match(line)
    if matches:
        matches_url = r_url.match(matches.groups()[5])
        if matches_url:
